...
class CategorizedDropdown<T> extends StatelessWidget {
  /// The list of items the user can select.
  ///
  /// If the [onChanged] callback is null or the list of items is null
  /// then the dropdown button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input.
  final List<CategorizedDropdownItem<T>>? items;
  ...
  @override
  Widget build(BuildContext context) {
    List<DropdownMenuItem<T>>? buildDrowdownItems() {
      if (items == null) {
        return null;
      }
      ...
      for (var i = 0; i < items!.length; i++) {
        ...
        dropdownItems.add(DropdownMenuItem<T>(
          enabled: element.subItems == null,
          value: element.value,
          child: element.text != null
              ? Text(element.text!, style: TextStyle(fontWeight: element.subItems == null ? null : FontWeight.bold))
              : element.widget!,
        ));

        if (element.subItems != null) {
          for (var j = 0; j < element.subItems!.length; j++) {
            final item = element.subItems![j];
